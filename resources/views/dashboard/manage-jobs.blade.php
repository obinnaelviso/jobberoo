@extends('layout.main')
@section('title', 'Manage Jobs - Jobberoo')
            ...
            <table class="table table-hover" id="table">
                ...
                @foreach($jobs as $job)
                  <tr>
                    ...
                    <td>
                        <button type="button" class="btn btn-primary py-2" data-toggle="modal" data-target="#view_{{ $job->id }}">View</button> |
                        <a href="{{ route('dashboard.manage.edit.job', $job->id) }}" class="btn btn-warning py-2">Edit</a> |
                        <form action="{{ route('dashboard.manage.job.status', $job->id) }}" method="POST" style="display: inline">
                            @csrf @method('put')
                                @if($job->status->name == 'open')
                                    <input type="submit" class="btn btn-danger" name="status" value="Close Job"/>
                                @else
                                    <input type="submit" class="btn btn-success" name="status" value="Open Job"/>
                                @endif
                        </form>
                    </td>
                  </tr>
